class ScalableSpriteButtonWidget : ICheckableWidget, Widget
{
	...
	void DoDraw(SpriteBatch& sb, vec2 pos) override
	{
		...
		if (!IsEnabled())
		{
			@left = m_spriteDisabled[0];
			@mid = m_spriteDisabled[1];
			@right = m_spriteDisabled[2];
		}
		else
		{
			...
			{
				@left = m_spriteHover[0];
				@mid = m_spriteHover[1];
				@right = m_spriteHover[2];
			}
		}
		...
		if (m_text !is null)
		{
			...
			vec2 textPos;
			if (m_textOffset.x == 0)
				textPos.x = m_width / 2.0f - m_text.GetWidth() / 2.0f;
			else

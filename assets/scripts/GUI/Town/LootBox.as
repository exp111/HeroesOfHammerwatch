...
class LootBox : ScriptWidgetHost
{
	...
	void UpdateInterface()
	{
		...
		m_wPlayButton.m_enabled = (gm.m_townLocal.m_gold >= g_LootBoxCost);
	}

	void Play()
	{
		auto gm = cast<Campaign>(g_gameMode);
		if (gm.m_townLocal.m_gold < g_LootBoxCost)
		{
			PrintError("Not enough gold!");
			return;
		}

		gm.m_townLocal.m_gold -= g_LootBoxCost;
		...
		if (m_waitTime > 0)
		{
			...
			if (m_waitTime <= 0)
			{
				...
				for (uint i = 0; i < record.items.length(); i++)
				{
					...
					m_itemWidgets[i].m_wItem.Set(newItem);

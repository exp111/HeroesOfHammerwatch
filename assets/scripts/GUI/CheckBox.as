class CheckBoxWidget : ICheckableWidget, Widget
{
	...
	void Load(WidgetLoadingContext &ctx) override
	{
		...
		{
			@m_spriteUnchecked = def.GetSprite(spriteset);
			@m_spriteHover = def.GetSprite(spriteset + "-hover");
			@m_spriteChecked = def.GetSprite(spriteset + "-checked");
			@m_spriteDisabled = def.GetSprite(spriteset + "-disabled");
		}
	...
	void DoDraw(SpriteBatch& sb, vec2 pos) override
	{
		...
		else
		{
			...
			if (m_checked)
				@sprite = m_spriteChecked;
			else if (m_hovering && m_spriteHover !is null)
				@sprite = m_spriteHover;

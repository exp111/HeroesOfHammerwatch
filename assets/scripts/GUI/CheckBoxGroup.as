...
class CheckBoxGroupWidget : ScrollableWidget
{
	...
	ICheckableWidget@ FindCheckable(Widget@ w)
	{
		...
		{
			ICheckableWidget@ ret = FindCheckable(w.m_children[i]);
			if (ret !is null)
				return ret;
		}
	...
	void AddChild(ref widget, int insertAt = -1) override
	{
		...
		ICheckableWidget@ checkable = FindCheckable(cast<Widget>(widget));
		if (checkable !is null)
		{
			...
			PrintError("Widget or its children are not of type ICheckableWidget");
	}

class SpeechBubble
{
	...
	void SetText(string title, string text)
	{
		if (title != "")
		{
			auto fontTitle = Resources::GetBitmapFont("gui/fonts/arial11_bold_nooutline.fnt");
			@m_title = fontTitle.BuildText(title, 200, TextAlignment::Center);
			m_title.SetColor(m_titleColor);
		}
		else
			@m_title = null;
		...
		if (text != "")
		{
			auto fontText = Resources::GetBitmapFont("gui/fonts/arial11_nooutline.fnt");
			@m_text = fontText.BuildText(text, 200, TextAlignment::Center);
			m_text.SetColor(m_textColor);
		}
		else
			@m_text = null;
	...
	void Draw(SpriteBatch& sb, int idt)
	{
		...
		if (m_spriteArrow !is null)
			m_spriteArrow.Draw(sb, pos + vec2(size.x / 2 - m_spriteArrow.GetWidth() / 2, size.y - 1), g_menuTime);

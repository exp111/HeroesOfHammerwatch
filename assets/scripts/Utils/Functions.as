...
string formatTime(float tm, bool fractions = false, bool forceMinutes = false, bool forceHours = false, bool letters = false)
{
	string str = "";
...
bool rayQuickFromUnit(UnitPtr unit, vec2 toPos, uint slots, RaycastType type)
{
	return rayQuickWithoutUnit(xy(unit.GetPosition()), toPos, slots, type, unit);
}

bool rayQuickWithoutUnit(vec2 fromPos, vec2 toPos, uint slots, RaycastType type, UnitPtr unit)
{
	array<RaycastResult>@ res = g_scene.Raycast(fromPos, toPos, slots, type);
	for (uint i = 0; i < res.length(); i++)
	{
		UnitPtr hitUnit = res[i].FetchUnit(g_scene);
		if (hitUnit != unit)
			return true;
	}
	return false;

namespace Upgrades
{
	...
	void LoadShop(string id)
	{
		auto svShop = Resources::GetSValue("tweak/shops/" + id + ".sval");
		string className = GetParamString(UnitPtr(), svShop, "class");

		auto newShop = cast<Shop>(InstantiateClass(className, svShop));
		if (newShop is null)
		{
			PrintError("Failed to create shop class \"" + className + "\" for shop id \"" + id + "\"");
			return;
		}
	...
	void LoadShops()
	{
		...
		LoadShop("apothecary");
		LoadShop("trainer");
		LoadShop("generalstore");
		LoadShop("dungeonstore");
		LoadShop("chapel");
		LoadShop("magicshop");
		LoadShop("oretrader");
	}

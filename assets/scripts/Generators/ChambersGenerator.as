...
class ChambersGenerator : DungeonGenerator
{
	...
	void Generate(Scene@ scene) override
	{
		while (true)
		{
			...
			PlotRect(ivec2(m_botRoom.x - 10, m_botRoom.y + corridorLen), ivec2(m_botRoom.x + 10, m_botRoom.y + corridorLen + 20), Cell::Floor);
			
			ivec2 startPos = ivec2(m_botRoom.x - 5, m_botRoom.y + corridorLen + 10 - 3);
		...
		int dist = int(max(Width, Height) / 2.0f * 1.5f);

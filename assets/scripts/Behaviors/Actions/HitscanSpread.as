...
class HitscanSpread : IAction
{
	...
	HitscanSpread(UnitPtr unit, SValue& params)
	{
		...
		m_dmg = GetParamInt(unit, params, "dmg");
		m_dmgType = GetParamDamageType(unit, params, "dmg-type", true);
	...
	void DoDamageUnits(Actor@ owner, bool husk)
	{
		...
		{
			m_hitUnits[i].unit.TriggerCallbacks(UnitEventType::Damaged);
			
			auto dmgTaker = cast<IDamageTaker>(m_hitUnits[i].unit.GetScriptBehavior());
			if (dmgTaker is null)
				continue;
			
			if (!FilterHuskDamage(owner, m_hitUnits[i].unit, husk))
				continue;
			
			float mul = owner.GetDamageMulTarget(false, m_hitUnits[i].unit) * m_hitUnits[i].intensity;
			
%if MOD_ROGUELIKE
			mul *= randf() * 0.5 + 0.75;
%endif
			
			int dmg = damage_round(m_dmg * mul * m_hitUnits[i].numHits);
			if (dmg > 0)
				dmgTaker.Damage(DamageInfo(m_dmgType, owner, dmg, false, true, m_weaponInfo), m_hitUnits[i].pos, m_hitUnits[i].dir);
		}
	...
	vec2 ShootSpreadHitscan(Actor@ owner, vec2 from, vec2 to, float intensity, bool husk, int ricochet = 0, float ricochetMul = 1.0f)
	{
		...
		for (uint j = 0; j < results.length(); j++)
		{
			...
			if (dt !is null && dt.ShootThrough(res.point, dir))
				continue;
			...
			if (m_effects is null || ApplyEffects(m_effects, owner, res_unit, res.point, dir, intensity, husk, 0, m_teamDmg))
			{
				bool toAdd = true;
				for (uint i = 0; i < m_hitUnits.length(); i++)
				{

...
class HwSpawnUnit : IAction
{
	...
	bool DoAction(SValueBuilder@ builder, Actor@ owner, Actor@ target, vec2 pos, vec2 dir, float intensity)
	{
		...
		else
		{
			...
			{
				bool canSpawn = true;
				auto res = g_scene.QueryRect(pos, 1, 1, ~0, RaycastType::Shot);
			...
			{
				if (!g_scene.RaycastQuick(pos, pos + dir, ~0, RaycastType::Shot))
					DoSpawnUnit(builder, pos + dir);
				else if (!g_scene.RaycastQuick(pos, vec2(pos.x, pos.y - m_spawnDist), ~0, RaycastType::Shot))
					DoSpawnUnit(builder, vec2(pos.x, pos.y - m_spawnDist));
				else if (!g_scene.RaycastQuick(pos, vec2(pos.x, pos.y + m_spawnDist), ~0, RaycastType::Shot))
					DoSpawnUnit(builder, vec2(pos.x, pos.y + m_spawnDist));
				else if (!g_scene.RaycastQuick(pos, vec2(pos.x - m_spawnDist, pos.y), ~0, RaycastType::Shot))
	...
	void DoSpawnUnit(SValueBuilder@ builder, vec2 pos)
	{
		...
		UnitPtr unit = m_entries[iProd].m_prod.Produce(g_scene, xyz(pos));

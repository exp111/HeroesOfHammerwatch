...
*/
	void Trigger()
	{
		...
		{
			auto actor = cast<Actor>(insideActors[i].GetScriptBehavior());
			if (actor.Team != m_owner.Team)
				totalDamage += actor.Damage(DamageInfo(m_owner, 0, m_damage, false, true, 0), xy(actor.m_unit.GetPosition()), vec2());
			else
